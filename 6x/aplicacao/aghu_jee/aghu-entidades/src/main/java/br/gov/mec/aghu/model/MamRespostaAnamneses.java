...
@Entity
@Table(name = "MAM_RESPOSTA_ANAMNESES", schema = "AGH")
public class MamRespostaAnamneses extends BaseEntityId<MamRespostaAnamnesesId> implements java.io.Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = -3848201119495841726L;
	private MamRespostaAnamnesesId id;
	...
	public MamRespostaAnamneses() {
	}

	public MamRespostaAnamneses(MamRespostaAnamnesesId id) {
		this.id = id;
	}
	...
	public MamRespostaAnamnesesId getId() {
		return this.id;
	}

	public void setId(MamRespostaAnamnesesId id) {
		this.id = id;
	}
	...
	public enum Fields {
		...
		PLP_PAC_CODIGO("plpPacCodigo"), PLP_SEQP("plpSeqp"), ANA_SEQ("id.anaSeq"),
		QUS_QUT_SEQ("id.qusQutSeq"), QUS_SEQP("id.qusSeqp"), SEQP("id.seqp"),
		RESPOSTA("resposta"), VALOR_VALIDO("mamValorValidoQuestao"),
		MAM_QUESTAO("mamQuestao"), MAM_ANAMNESES("mamAnamneses"),
		VVQ_QUS_QUT_SEQ("vvqQusQutSeq"), VVQ_QUS_SEQP("vvqQusSeqp"),
		VVQ_SEQP("vvqSeqp");
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof MamRespostaAnamneses)) {
			return false;
		}
		MamRespostaAnamneses other = (MamRespostaAnamneses) obj;
		if (getId() == null) {
			if (other.getId() != null) {
				return false;
			}
	...
	@ManyToOne(fetch = FetchType.LAZY)
	@JoinColumn(name = "ANA_SEQ", nullable = false, insertable = false, updatable = false)
