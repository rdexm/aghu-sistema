...
public class AjustadorClassesNegocioInjectDAO {
		...
		if (verificarProcessaArquivo(file)) {
			...
			while (indexNew != -1) {		
				...
				if (indexAbreColchetes == -1 || (indexAbreParenteses != -1 && indexAbreParenteses < indexAbreColchetes)) {
					...
					if (classeInstanciacao.endsWith("DAO")
			...
			if (sbAtributos.length() > 0 && !stNovoConteudo.contains("import javax.inject.Inject;")) {
				stNovoConteudo = stNovoConteudo
						.replace("import javax.ejb.Stateless;",
