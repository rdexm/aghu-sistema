...
public class CadastroAtestadoFgtsPisPasepController extends ActionController {
...
//	private RelatorioAtestadosController relatorioAtestadosController;
	...
	public void gravar(){
		try {
			...
			this.ambulatorioFacade.persistirMamAtestado(this.getMamAtestado(), true);
	...
	public void editar(){
		...
		definirComposDeTexto(this.mamAtestado.getIndMotivoUsoFgts(), mpmAltaSumario.getPaciente().getNome(), mpmAltaSumario.getPaciente().getProntuario());
	...
	public void excluir(){
		try {
			this.ambulatorioFacade.excluirAtestadoFgts(itemSelecionado);
	...
	public void limpar() {		
		...
		definirComposDeTexto(null, mpmAltaSumario.getPaciente().getNome(), mpmAltaSumario.getPaciente().getProntuario());
	}	
	...
	private String[] montarArrayTextos(DominioIndMotivoUsoFgts indMotivoUsoFgts, String nome, Integer prontuario) {
		...
		if(indMotivoUsoFgts == null){
			...
			textos[4] = this.getBundle().getString("LABEL_ATESTADO_FGTS_CID10");
...
//				this.relatorioAtestadosController.setDescricaoDocumento("FGTS e PIS/PASEP");
