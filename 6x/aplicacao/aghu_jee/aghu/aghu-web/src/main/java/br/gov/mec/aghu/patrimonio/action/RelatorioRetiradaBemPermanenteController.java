...
public class RelatorioRetiradaBemPermanenteController extends ActionReport {
	...
	private static final Log LOG = LogFactory.getLog(RelatorioRetiradaBemPermanenteController.class);	
	...
	protected void inicializar() {
		...
		try {
			servidor = patrimonioFacade.obterServidorPorUsuario(obterLoginUsuarioLogado());
	...
	protected Map<String, Object> recuperarParametros() {
		...
		Integer nrpSeq = null;
		Integer nroItem = null;
		
		nrpSeq = itemRetiradaList.get(0).getRecebimento();
		nroItem = itemRetiradaList.get(0).getItemRecebimento();
		
		tecnicoItemRecebimento = patrimonioFacade.buscarTecnicoResponsavelPorMatENome(nrpSeq, nroItem);
				
		if(tecnicoItemRecebimento != null){
			if (tecnicoItemRecebimento.getMatricula()!= null){
				tecnicoResponsavel = tecnicoItemRecebimento.getMatricula().toString();
			}
			if (tecnicoItemRecebimento.getNome() != null){
	...
	@Override
	protected List<AceiteTecnicoParaSerRealizadoVO> recuperarColecao()throws ApplicationBusinessException {
		if(cameFrom.equals("REIMPRIMIR_PROTOCOLO")){
			return itemRetiradaList;
		}else{
			AceiteTecnicoParaSerRealizadoVO aceite = itemRetiradaList.get(posicao);
			List<AceiteTecnicoParaSerRealizadoVO> retorno = new ArrayList<AceiteTecnicoParaSerRealizadoVO>();
			retorno.add(aceite);
			return retorno;
		}
