...
(function($) {
	...
	Timepicker.prototype = {
		...
		addTimePicker: function(dp_inst) {
			// Se o ID do campo tiver o caractere ":", retira a ultima parte
			var tp_inst = this;
			var currDT;
		...
		injectTimePicker: function(dp_inst, tp_inst) {
			...
			if ($dp.find("div#ui-timepicker-div-"+ dp_inst.jid).length === 0) {
				var noDisplay = ' style="display:none;"';
				var html =
					'<div class="ui-timepicker-div" id="ui-timepicker-div-' + dp_inst.jid + '"><dl>' +
						'<dt class="ui_tpicker_time_label" id="ui_tpicker_time_label_' + dp_inst.jid + '"'	+ ((opts.showTime)   ? '' : noDisplay) + '>' + opts.timeText + '</dt>' +
						'<dd class="ui_tpicker_time" id="ui_tpicker_time_' + dp_inst.jid + '"'	+ ((opts.showTime)   ? '' : noDisplay) + '><input type="text" size="6" class="timeFieldObject"></dd>' +
						'<dt class="ui_tpicker_hour_label" id="ui_tpicker_hour_label_' + dp_inst.jid + '"'   + ((opts.showHour)   ? '' : noDisplay) + '>' + opts.hourText + '</dt>';
						
				if (opts.hourGrid > 0)
				{
					html += '<dd style="display:none;" class="ui_tpicker_hour ui_tpicker_hour_' + opts.hourGrid + '">' +
							'<div id="ui_tpicker_hour_' + dp_inst.jid + '"' + ((opts.showHour)   ? '' : noDisplay) + '></div>' +
							'<div><table><tr>';
				...
				{
					html += '<dd class="ui_tpicker_hour" id="ui_tpicker_hour_' + dp_inst.jid + '"'	+ ((opts.showHour)   ? '' : noDisplay) + '></dd>';
				}
						
				html += '<dt class="ui_tpicker_minute_label" id="ui_tpicker_minute_label_' + dp_inst.jid + '"'	+ ((opts.showMinute) ? '' : noDisplay) + '>' + opts.minuteText + '</dt>';
				
				if (opts.minuteGrid > 0)
				{
					html += '<dd style="display: none;" class="ui_tpicker_minute ui_tpicker_minute_' + opts.minuteGrid + '">' +
							'<div id="ui_tpicker_minute_' + dp_inst.jid + '"' + ((opts.showMinute) ? '' : noDisplay) + '></div>' +
							'<div><table><tr>';
				...
				{
					html += '<dd class="ui_tpicker_minute" id="ui_tpicker_minute_' + dp_inst.jid + '"'	+ ((opts.showMinute) ? '' : noDisplay) + '></dd>'
				}
				
				html += '<dt class="ui_tpicker_second_label" id="ui_tpicker_second_label_' + dp_inst.jid + '"'	+ ((opts.showSecond) ? '' : noDisplay) + '>' + opts.secondText + '</dt>';
				
				if (opts.secondGrid > 0)
				{
					html += '<dd class="ui_tpicker_second ui_tpicker_second_' + opts.secondGrid + '">' +
							'<div id="ui_tpicker_second_' + dp_inst.jid + '"' + ((opts.showSecond) ? '' : noDisplay) + '></div>' +
							'<table><table><tr>';
				...
				{
					html += '<dd class="ui_tpicker_second" id="ui_tpicker_second_' + dp_inst.jid + '"'	+ ((opts.showSecond) ? '' : noDisplay) + '></dd>';
				}
				...
				tp_inst.hour_slider = $tp.find('#ui_tpicker_hour_'+ dp_inst.jid).slider({
				...
				tp_inst.minute_slider = $tp.find('#ui_tpicker_minute_'+ dp_inst.jid).slider({
					orientation: "horizontal",
				...
				tp_inst.second_slider = $tp.find('#ui_tpicker_second_'+ dp_inst.jid).slider({
					orientation: "horizontal",
				...
				tp_inst.$timeObj = $('#ui_tpicker_time_'+ dp_inst.jid+' > .timeFieldObject');
				
				var hourFieldKeyControl = false;
				
				$(tp_inst.$timeObj).bind('keydown', function(e){
					var key = e.which;
					if (key == 8 || key == 9 || key == 37 || key == 39 || key == 46 || key == 186) {
						hourFieldKeyControl = true;
						return true;
					}
					...
					hourFieldKeyControl = false;
					return true;
				});
				
				$(tp_inst.$timeObj).bind('keyup', function(e){
					if (hourFieldKeyControl == false) {
						var value = ($(this).val());
						if (value.length == 2) {
							$(this).val(value+':');
						}
				...
				$(tp_inst.$timeObj).bind('change', function(){
					var time = $(this).val();
					time = time.split(':');
					var hour = time[0];
					var minute = time[1];
					var second = time[2];
					tp_inst.hour_slider.slider("option", "value", hour);
					tp_inst.minute_slider.slider("option", "value", minute);
					tp_inst.second_slider.slider("option", "value", second);
					tp_inst.onTimeChange(dp_inst, tp_inst);
				});
		...
		//########################################################################
		onTimechangeField: function() {
			var time = tp_inst.$timeObj.val();
			var time = time.split(':');
			var hour   = time[0];
			var minute = time[1];
			var second = time[2];
		...
		onTimeChange: function(dp_inst, tp_inst) {
			...
			tp_inst.$timeObj.val(tp_inst.formattedTime);
