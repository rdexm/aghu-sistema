...
import br.gov.mec.aghu.model.VAfaPrcrDisp;

public class VAfaPrcrDispDAO extends br.gov.mec.aghu.core.persistence.dao.BaseDao<VAfaPrcrDisp> {
	...
	private DetachedCriteria getCriteriaPesquisarPacientesParaDispensacao(Date dataReferencia, AghUnidadesFuncionais farmacia,
		...
		DetachedCriteria criteria = DetachedCriteria.forClass(VAfaPrcrDisp.class, "apd");
		
		criteria.createAlias("atendimento"/*VAfaPrcrDisp.Fields.ATENDIMENTO.toString()*/, "atendimento", JoinType.LEFT_OUTER_JOIN);
		criteria.createAlias("atendimentoLocal"/*VAfaPrcrDisp.Fields.ATENDIMENTO_LOCAL.toString()*/, "atendimentoLocal", JoinType.LEFT_OUTER_JOIN);
		
		criteria.createAlias("atendimento."+AghAtendimentos.Fields.PACIENTE.toString(), "PACIENTE", JoinType.LEFT_OUTER_JOIN);
		
		criteria.add(Restrictions.gt(VAfaPrcrDisp.Fields.DT_REFERENCIA.toString(), dataInicial));
		criteria.add(Restrictions.le(VAfaPrcrDisp.Fields.DT_REFERENCIA.toString(), dataFinal));
		
		if(paciente!=null){
			criteria.add(Subqueries.propertyIn(VAfaPrcrDisp.Fields.ATD_SEQ.toString(), getCriteriaPaciente(paciente)));
		}
		
		criteria.add(Restrictions.or(
				Restrictions.and(
						Subqueries.propertyIn(VAfaPrcrDisp.Fields.ATD_SEQ.toString(), getCriteriaDispMdto(AfaDispensacaoMdtos.Fields.PRESCRICAO_MEDICA_ATD_SEQ, 		farmacia, false)), 
						Subqueries.propertyIn(VAfaPrcrDisp.Fields.SEQ.toString(), 	 getCriteriaDispMdto(AfaDispensacaoMdtos.Fields.PRESCRICAO_MEDICA_SEQ, 	farmacia, false)))
				,
				Restrictions.and(
					Subqueries.propertyIn(VAfaPrcrDisp.Fields.ATD_SEQ.toString(), getCriteriaDispMdto(AfaDispensacaoMdtos.Fields.IMO_PMO_PTE_ATD_SEQ, 	farmacia, true)), 
					Subqueries.propertyIn(VAfaPrcrDisp.Fields.SEQ.toString(), 	 getCriteriaDispMdto(AfaDispensacaoMdtos.Fields.IMO_PMO_PTE_SEQ, 		farmacia, true)))
					));
		
		criteria.add(Subqueries.propertyIn(VAfaPrcrDisp.Fields.ATD_SEQ_LOCAL.toString(), getCriteriaLeitoEQuarto(leito, quarto, unidadeFuncionalPrescricao, paciente)));
		
		return criteria;
	}
	
	public List<VAfaPrcrDisp> pesquisarPacientesParaDispensacao(Integer firstResult,
	...
	private DetachedCriteria getCriteriaLeitoEQuarto(AinLeitos leito,
		...
		String atdSeqLocalMain = "apd."+VAfaPrcrDisp.Fields.ATD_SEQ_LOCAL.toString();
	...
	private DetachedCriteria getCriteriaDispMdto(AfaDispensacaoMdtos.Fields propertyAfaDispensacao, AghUnidadesFuncionais farmacia, Boolean comImo) {
		...
		String atdSeqMain = "apd." + VAfaPrcrDisp.Fields.ATD_SEQ.toString(); //apd.id.atdSeq
		String pmeSeqMain = "apd." + VAfaPrcrDisp.Fields.SEQ.toString(); //apd.id.seq
