...
public class AGHUITextFontResolver implements FontResolver {
    ...
    public AGHUITextFontResolver(SharedContext sharedContext) {
    ...
    public void addFont(String path, String encoding, boolean embedded, String pathToPFB) throws DocumentException, IOException {
        ...
        if (lower.endsWith(".otf") || lower.endsWith(".ttf") || lower.indexOf(".ttc,") != -1) {
            ...
            try {
                AGHUTrueTypeUtil.populateDescription(path, font, descr);
            } catch (Exception e) {
    ...
    private void addFontFaceFont(
        ...
        if (lower.endsWith(".otf") || lower.endsWith(".ttf") || lower.indexOf(".ttc,") != -1) {
            ...
            FontDescription descr = new FontDescription(font);
            try {
                AGHUTrueTypeUtil.populateDescription(uri, afmttf, font, descr);
            } catch (Exception e) {
                throw new XRRuntimeException(e.getMessage(), e);
            }
    ...
    private FSFont resolveFont(SharedContext ctx, String fontFamily, float size, IdentValue weight, IdentValue style, IdentValue variant) {
        ...
        if (result != null) {
            return new AGHUITextFSFont(result, size);
        }
    ...
    private static void addCourier(Map result) throws DocumentException, IOException {
    ...
    private static void addTimes(Map result) throws DocumentException, IOException {
    ...
    private static void addHelvetica(Map result) throws DocumentException, IOException {
