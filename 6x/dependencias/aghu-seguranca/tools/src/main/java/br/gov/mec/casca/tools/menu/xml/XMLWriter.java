...
public class XMLWriter {
	...
	private void gerarMenus(List<Menu> menus, PrintWriter writer, String ident) {
		for (Menu menu : menus) {
			...
			writer.printf("%s<menu nome=\"%s\"", ident, menu.getNome());
			
			if (menu.getUrl() != null) {
				writer.printf(" url=\"%s\"", menu.getUrl());
			}
			
			writer.printf(" ordem=\"%s\" ativo=\"%s\"", menu.getOrdem(), menu.getAtivo().equals(DominioSimNao.S));
			
			if (menu.getClasseIcone() != null) {
				writer.printf(" classe-icone=\"%s\"", menu.getClasseIcone());
			}
			
			writer.printf(" aplicacao=\"%s\"", menu.getAplicacao().getNome());
